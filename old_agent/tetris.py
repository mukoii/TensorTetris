...
class Tetris:
    ...
    def __init__(self, _rows=14, _cols=6, _render=False):
        ...
        self._observation_spec = array_spec.BoundedArraySpec(
            ...
            dtype=np.float32,
            minimum=0.0,
            maximum=float(max(_rows, _cols, 6)),  # generous upper bound
            name="observation",
        )
    ...
    def reset(self):
        ...
        self.pos = [1, 1] # Middle of top of board
        self.rotation = 0
        self.steps = 0
    ...
    def observation_spec(self):
        ...
        tempArray = self.get_board_copy().flatten()

        # print(f"Array:{tempArray}")
        tempArray = np.append(tempArray, self.next_piece)
        self.observation_spec.from_array(tempArray)

        return self.observation_spec
        
    def action_spec(self):
        action_spec = array_spec.BoundedArraySpec(
        shape=(), dtype=np.int32, minimum=0, maximum=self.config["cols"] * 4 - 1, name='action')
    ...
    def _add_piece(self):
        ...
        # print(f"Piece:{self.current_piece}, Rotation:{self.rotation}")
    ...
    def step_col(self, col, rotation):
        self.steps += 1
        # print(f"Placing at: {col}. At Rotation {rotation}")
    ...
    def prefill_board(self, rows):
        """Fill bottom N rows randomly, leaving one random gap per row."""
        """for row in range(self.rows - rows, self.rows):
            gap = np.random.randint(0, self.cols)
